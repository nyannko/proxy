...
class MultiProxy(TunnelCommunity):
    ...
    def check_circuit(self):
        if self.circuits != {}:
            ...
            # update next hop in socks5 factory(send to forward factory)
            self.socks5_factory.circuit_peers[cir_id] = peer_address
            print "updated value", self.socks5_factory.circuit_peers[cir_id]
            print "self circuit", self.circuits
        if self.exit_candidates != {}:
            ...
            # update exit layer, how to update layer if it goes down
...
class Socks5Protocol(protocol.Protocol):
    ...
    def send_address(self, data):
        ...
        print "send address debug!", self.socks5_factory.circuit_peers
